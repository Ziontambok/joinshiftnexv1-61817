...
const industries = [
  {
    icon: <ShoppingCart size={48} className="text-shiftnex-teal" />,
    title: "E-commerce",
  ...
  {
    icon: <Home size={48} className="text-shiftnex-teal" />,
    title: "Real Estate",
  ...
  {
    icon: <Scale size={48} className="text-shiftnex-teal" />,
  ...
  {
    icon: <Heart size={48} className="text-shiftnex-teal" />,
  ...
  {
    icon: <Briefcase size={48} className="text-shiftnex-teal" />,
    ...
    <section id="industries" className="py-24 bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-20 reveal">
          <span className="text-shiftnex-teal font-medium text-sm uppercase tracking-wider mb-3 inline-block">Industries We Serve</span>
          <h2 className="text-4xl md:text-5xl font-bold text-shiftnex-deep-blue mb-6">Specialized Support for Your Industry</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {industries.map((industry, index) => (
            <div 
              key={index}
              className="bg-white p-8 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 hover:border-shiftnex-teal/30 transition-all duration-300 hover:-translate-y-1 reveal"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="bg-shiftnex-teal/10 w-20 h-20 rounded-full flex items-center justify-center mb-6 mx-auto">
                {industry.icon}
              </div>
              
              <h3 className="text-2xl font-bold text-shiftnex-deep-blue mb-4 text-center">{industry.title}</h3>
              <p className="text-gray-600 leading-relaxed mb-6 text-center">{industry.description}</p>
              
              <div className="border-t border-gray-200 pt-4">
                <p className="text-sm font-semibold text-shiftnex-deep-blue mb-3 text-center">Key Services:</p>
                <ul className="space-y-2">
                  {industry.services.map((service, idx) => (
                    <li key={idx} className="text-sm text-gray-600 flex items-center justify-center">
                      <span className="w-1.5 h-1.5 bg-shiftnex-teal rounded-full mr-2"></span>
                      {service}
                    </li>
                  ))}
                </ul>
        ...
        <div className="mt-16 text-center bg-gradient-to-r from-soft-teal/10 to-soft-blue/10 rounded-2xl p-8 max-w-4xl mx-auto reveal">
          <h3 className="text-2xl font-bold text-shiftnex-deep-blue mb-4">Don't See Your Industry?</h3>
          ...
          <a 
            ...
            className="bg-shiftnex-teal text-white px-8 py-3 rounded-lg hover:bg-opacity-90 transition-all inline-block"
          >
            Contact Us
          </a>
        </div>
